...
\end{table}
\noindent Die Idempotenz ist besonders wichtig: Ein Worker könnte nach einem Retry
...
\end{itemize}
\noindent Der Client kann \texttt{GetResult} beliebig oft aufrufen, bis der Status terminal ist.
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.58]{AblaufdiagrammFehler.drawio.png}
	\caption{Fehlerfall-Diagramm}
\end{figure}
...
\noindent \textbf{Beispiele aus dem laufenden System:}
...
auf Port \path{STATUS_HTTP_PORT} (Standard: 8080).
Der Endpoint lässt sich direkt mit \path{curl http://localhost:8080/status} abfragen.

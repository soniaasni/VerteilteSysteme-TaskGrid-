...
\begin{table}[H]
\centering
\begin{tabular}{m{4.4cm} m{2.2cm} m{6.75cm} m{2cm}}
\toprule
...
\end{table}
\noindent Die Längenbeschränkungen (\texttt{MAX\_TYPE\_LEN = 32}, \texttt{MAX\_PAYLOAD\_LEN = 1024},
...
in\\
\texttt{src/common/protocol.py} erzeugt:
...
\noindent Der Zähler wird als String gespeichert, ist aber int32-kompatibel --
...
\noindent Jeder Übergangsversuch der nicht in \texttt{VALID\_TRANSITIONS} gelistet ist,
wirft eine \\
\texttt{InvalidTransitionError}-Exception.
...
\subsection{Nebenläufigkeit im Dispatcher}
